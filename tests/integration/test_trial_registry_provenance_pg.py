...
the EXISTING call shape is byte-for-byte unchanged (kwargs default None,
columns land NULL — honest history), and the new kwargs persist so a future
backtest CAN pin its hypothesis and feature-store dataset vintage."""
...
def test_existing_call_shape_unchanged_null_provenance(pg_session):
    s = pg_session
    rid = register_trial(s, family="prov-test", spec={"v": 1},
                         metrics={"sharpe": 0.1})
    row = s.execute(text(
...
def test_new_kwargs_are_persisted(pg_session):
    s = pg_session
    rid = register_trial(
        s, family="prov-test", spec={"v": 2}, metrics={"sharpe": 0.2},
