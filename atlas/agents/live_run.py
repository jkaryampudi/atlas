...
from atlas.dcp.signals.pead.generate import extract_pead_signal_evidence
...
def build_evidence(s: Session, symbol: str) -> list[tuple[str, str]]:
    ...
    # 8. SIGNALS — momentum (ADR-0010) and PEAD (ADR-0013): the active
    #    quant.signals row for the symbol from each paper-approved satellite
    #    strategy — rank, ranked signal value, validity, and the REAL signal
    #    UUID in the ref, so a BUY memo citing it is constitutionally grounded
    #    and the bridge attaches the true lineage (and the ADR-0014 sleeve
    #    budget attributes the proposal to the right sleeve)
    for block in (extract_fundamentals_evidence(s, symbol, on=on),
                  ...
                  extract_pead_signal_evidence(s, symbol, on=on)):
