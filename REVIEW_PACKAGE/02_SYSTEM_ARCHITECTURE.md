...
| DB roles `agent_reader`/`dcp_writer`/`audit_writer`; agents "literally cannot write risk limits" | Single DB role; the wall is enforced by *import isolation + schema*, not DB grants | **high** — a real defense-in-depth layer the design promised is absent |
